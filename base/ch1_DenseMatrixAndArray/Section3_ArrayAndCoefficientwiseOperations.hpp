...
// Eigen还为某些常见情况提供了typedef，其方式类似于Matrix
// typedef，但有一些细微的差异 因为单词“ array”既用于一维数组，也用于二维数组.
...
void AdditionAndSubtraction()
{
    ...
    // 数组还支持以下形式的表达式，该表达式array +
    // scalar将标量添加到数组中的每个系数。
...
void ConvertingBetweenArrayAndMatrixExpressions()
{
    ...
    // 例如，语句需要两个矩阵和，他们两个转换为阵列，用来将它们相乘系数明智并将结果指定给矩阵变量（这是合法的，因为本征允许分配数组表达式到矩阵的变量）。result
    // = m.array() * n.array()mnresult

    // 实际上，这种使用情况非常普遍，以至于Eigen为矩阵提供了const
    // .cwiseProduct()方法来计算系数乘积。
    LOG();
    MatrixXf m(2, 2);
    MatrixXf n(2, 2);
    MatrixXf result(2, 2);
